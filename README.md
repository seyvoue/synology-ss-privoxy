...
``` sh
sudo docker run -dt \
--name nas-shadowsocks-client \
--volume /volume1/docker/runtime/nas-shadowsocks-client/shadowsocks:/config \
--volume /volume1/docker/runtime/nas-shadowsocks-client/privoxy:/etc/privoxy \
-p 1080:1080 \
seyvoue/synology-ss-privoxy \
-m "ss-local" \
-s "-c /config/shadowsocks.json -b 0.0.0.0 -l 1080"
```
