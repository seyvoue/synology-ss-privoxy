...
LABEL maintainer="seyvoue <seyvoue@qq.com>"
...
RUN apk upgrade \
    ...
    && rm -rf /linux-headers-4.4.6-r2.apk \
        ...
        /etc/service \
        /var/cache/apk/* \
    && echo "alias goproxy='export https_proxy=http://127.0.0.1:8118;export http_proxy=http://127.0.0.1:8118;'" >> /etc/profile \
    && echo "alias disproxy='unset http_proxy https_proxy'" >> /etc/profile \
    && source /etc/profile
...
VOLUME ["/etc/privoxy","${SS_CONFIG_DIR}"]
